...
**v1.13.4**. This document expands the Quick start in [`README.md`](../README.md). Prefer the published npm package for end users.
...
```bash
dsh plugin --profile web add /path/to/dsh-hub-oauth-gateway-1.13.4.tgz
```
...
- Install Hub `1.13.4` and Subscription `0.8.5` into the same **web profile**, then restart the existing DSH Web process once. The active runtime owner provides one Accounts & Models entry; Subscription restores its independent entry if Hub exits.
