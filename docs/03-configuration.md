...
| `gateway.opencodeGo.enabled` | `false` | Opt-in OpenCode Go compatibility: when true, gateway `POST /v1/chat/completions` proxies to pinned `https://opencode.ai/zen/go/v1/chat/completions` and injects sticky `x-opencode-session` (helps clients that omit session affinity / hit `MissingSessionID`). Session id preference: `x-deepseek-harness-session-id` → `x-opencode-session` → `x-session-id` → body `session_id` → UUID. Set the gateway Bearer key to an OpenCode API key. Toggle live from the Gateway tab or `PATCH` `{ "opencodeGoEnabled": true }` |
...
The coding OAuth settings UI opens at **Settings → Usage Center → Accounts**. It provides per-provider sign-in cards (device code, browser PKCE, or pasted redirect), multi-account add / set-default / remove (AuthDocument v2, max 8), **Import Claude Code** (preview → commit; macOS Keychain or file), GET-only provider quota summaries, model selection, and the allowlisted CLI credential pull wizard. Gateway controls remain optional, and compatibility diagnostics are available from the advanced Capabilities area rather than the first view.
