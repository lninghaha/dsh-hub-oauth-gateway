...
| `gateway.opencodeGo.enabled` | legacy only | True identifies migration-required state; it no longer forwards all chat requests. Apply explicit `opencodeGoRoute` after reviewing models, protocol and upstream credential reference. No generated session ID or inbound-key fallback. |
