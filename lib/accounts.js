...
function defaultAdapter(provider) {
	...
	if (providerId === "claude" || providerId === "claude-code") return "claude-oauth";
	if (providerId === "codex" || providerId === "openai-codex") return "codex-wham";
	if (providerId === "gemini" || providerId === "gemini-cli") return "gemini-quota";
	if (providerId === "copilot" || providerId === "github-copilot") return "copilot-device";
	if (providerId === "cursor") return "cursor-subscription";
	if (providerId === "grok" || providerId === "xai-grok") return "grok-subscription";
	if (providerId === "amp" || providerId === "ampcode") return "amp-subscription";
...
function adapterMode(adapter, monitor) {
	...
	if (["opencode-go", "zai-token-plan", "kimi-token-plan", "minimax-token-plan", "claude-oauth", "codex-wham", "gemini-quota", "copilot-device", "cursor-subscription", "grok-subscription", "amp-subscription"].includes(adapter)) return "subscription";
	return "balance";
}
...
/** Nearest reset among quota windows; prefers a future timestamp. */
export function nextResetAt(windows, now = Date.now()) {
	if (!Array.isArray(windows)) return null;
	const times = [];
	for (const window of windows) {
		const iso = toIso(window?.resetsAt);
	...
	return new Date(future.length > 0 ? Math.min(...future) : Math.min(...times)).toISOString();
}
		...
		const subscriptionId = spec.adapter === "zai-token-plan" ? "zai"
			: spec.adapter === "kimi-token-plan" ? "kimi"
				: spec.adapter === "minimax-token-plan" ? "minimax"
					: spec.adapter === "claude-oauth" ? "claude"
						: spec.adapter === "codex-wham" ? "codex"
							: spec.adapter === "gemini-quota" ? "gemini"
								: spec.adapter === "copilot-device" ? "copilot"
									: spec.adapter === "cursor-subscription" ? "cursor"
										: spec.adapter === "grok-subscription" ? "grok"
											: spec.adapter === "amp-subscription" ? "amp"
												: "opencode-go";
...
export function createAccountService({ credentials, getProviders, config = { monitors: {} }, deps = {} }) {
	...
	async function specs() {
		...
		if (deps.includeLegacyProviders !== false) {
			...
			if (!providers.some((provider) => provider.id === "claude" || provider.id === "claude-code")) providers.push({ id: "claude", displayName: "Claude", apiKeyEnv: "CLAUDE_OAUTH_TOKEN" });
			if (!providers.some((provider) => provider.id === "codex" || provider.id === "openai-codex")) providers.push({ id: "codex", displayName: "Codex", apiKeyEnv: "CODEX_ACCESS_TOKEN" });
			if (!providers.some((provider) => provider.id === "gemini" || provider.id === "gemini-cli")) providers.push({ id: "gemini", displayName: "Gemini", apiKeyEnv: "GEMINI_ACCESS_TOKEN" });
			if (!providers.some((provider) => provider.id === "copilot" || provider.id === "github-copilot")) providers.push({ id: "copilot", displayName: "GitHub Copilot", apiKeyEnv: "GITHUB_COPILOT_TOKEN" });
			if (!providers.some((provider) => provider.id === "dashscope" || provider.id === "bailian")) providers.push({ id: "dashscope", displayName: "DashScope", apiKeyEnv: "DASHSCOPE_API_KEY", baseURL: "https://dashscope.aliyuncs.com" });
			if (!providers.some((provider) => provider.id === "siliconflow" || provider.id === "silicon-flow")) providers.push({ id: "siliconflow", displayName: "SiliconFlow", apiKeyEnv: "SILICONFLOW_API_KEY", baseURL: "https://api.siliconflow.cn" });
			if (!providers.some((provider) => provider.id === "cursor")) providers.push({ id: "cursor", displayName: "Cursor", apiKeyEnv: "CURSOR_ACCESS_TOKEN" });
			if (!providers.some((provider) => provider.id === "grok" || provider.id === "xai-grok")) providers.push({ id: "grok", displayName: "Grok", apiKeyEnv: "GROK_ACCESS_TOKEN" });
			if (!providers.some((provider) => provider.id === "amp" || provider.id === "ampcode")) providers.push({ id: "amp", displayName: "Amp", apiKeyEnv: "AMP_API_KEY" });
		}
	...
	async function providerViews() {
		...
		return Promise.all((await specs()).map(async (spec) => {
			...
			return {
				...
				adapter: spec.adapter,
				credentialRef: spec.apiKeyRef ?? null,
				...
				nextResetAt: nextResetAt(windows, now),
				stale: account?.stale === true,
				...(account?.missingCredentials === void 0 ? {} : { missingCredentials: account.missingCredentials })
			};
		}));
	}
