...
const PROXY_ABI = "dsh.coding-oauth-core.proxy/v1";
...
function proxyStore() {
    ...
    if (existing !== undefined) {
        ...
        if (store.abi !== PROXY_ABI)
            throw new Error("coding OAuth proxy ABI is incompatible");
        return store;
    }
    const created = { abi: PROXY_ABI };
...
export function acquireCodingOAuthProxy(explicit, options = {}) {
    const url = explicit ??
    ...
    if (url === undefined)
        return { url: undefined, release: async () => undefined };
    const hosts = new Set(DEFAULT_PROXIED_HOSTS);
    if (options.proxyKimi === true) {
        for (const host of KIMI_PROXIED_HOSTS)
            hosts.add(host);
    }
    ...
    const active = { dispatcher, fallback, fingerprint, url, refs: 1 };
    store.active = active;
    setGlobalDispatcher(dispatcher);
    return proxyLease(store, active);
}
function proxyLease(store, active) {
    let released = false;
    return {
        url: active.url,
        async release() {
            ...
            released = true;
            active.refs -= 1;
            if (active.refs > 0 || store.active !== active)
                return;
            if (getGlobalDispatcher() === active.dispatcher)
                setGlobalDispatcher(active.fallback);
            delete store.active;
